...
💬 很多人会告诉你自动化或者工具（automation）不是 DevOps，但他们没有告诉你什么是 DevOps 的核心，如果你把自动化（automation）从他们的表述中除去，你看看 DevOps 还是什么呢。Ansible 从核心上讲，就是一个可编程的 automation 框架，通过各种扩展，完成自动化的各式各样任务，如创建用户，安装软件，等。
...
-   **自动化框架**（automation framework）：注意不只是 automation，这里其实包含了几重意思
    -   Ansible 是在 Python 上实现的，所以是可编程的自动话框架，实现自动化的扩展和重用；
    -   Ansbile 使用上采用类似 SQL“声明式”的 DSL，而不是一大段具体的自动化动作细节，关注点在“What”上而不是“How”，这是自动化层面的抽象，有别于脚本，使用者也不必纠缠于五花八门的命令行用法细节；
