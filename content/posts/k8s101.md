...
Kubernetes 的组件分成两种，通常相应运行在两种机器上：Master 控制节点负责管理集群，包含 `kube-apiserver`，`kube-controller-manager`，`kube-scheduler`，`etcd` 组件；Worker/Slave Node 节点运行具体的容器应用，由 `Container Runtime`，`kubelet` 和 `kube-proxy` 组成。
